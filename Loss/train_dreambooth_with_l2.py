...
from segmentation import sam
import shutil

loss_module = CLIP_TextImages_Loss()
w1, w2 = 0.5, 0.5 #0.3, 0.3
add_loss = 500

#folder_name = 'base_dog'
folder_name = 'denoised_both_0.3_500'
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                #print("loss is: ", loss) #loke
                
                #############loke
                #update loss
                #if in last few steps

                if step_count >= total_step/2: #add new loss at half
                    idx = step_count + step
                    #(args.max_train_steps - 1 - step_count)
                    image_denoised = latents_to_pil(noisy_latents - noise_pred)[0]
                    image_path = os.path.join(new_folder_path, f"denoised_image_{idx}.jpg")
                    image_denoised.save(image_path)
                    l1 = loss_module(args.instance_prompt, [image_path]) #instance prompt - image
                    #l1 = loss_module(args.instance_prompt, [image_denoised])
                    #print("l1 loss is : ", l1)
                    loss = loss + w1 * l1
                    
                        
                    #seg_image = sam(image_path) #segmentation or image_denoised
                    #seg_path = os.path.join(new_folder_path, f"seg_image_{idx}.jpg")
                    #seg_image.save(seg_path)
                    #l2 = loss_module(args.class_prompt, [seg_path]) #class prompt - segmented image
                    #print("l2 loss is : ", l2)
                    #loss = loss + w2 * l2 #w1/w2: weight for l1 & l2
