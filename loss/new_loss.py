...
##########
#add loss
from loss import CLIP_TextImages_Loss, load_image, pil_to_latents, latents_to_pil

loss_module = CLIP_TextImages_Loss()
w1, w2 = 1, 1
...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            pipeline = DiffusionPipeline.from_pretrained( #Loke: generate prior images
            ...
            for example in tqdm(
                ...
                images = pipeline(example["prompt"]).images #Loke: generate prior images
                # use prompt_embeds instead of example["prompt"]
        ...
        with TorchTracemalloc() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    latents = vae.encode(batch["pixel_values"].to(dtype=weight_dtype)).latent_dist.sample() #Loke: vae encoder
                    ...
                    #Loke: noised latent (latent + noise)
                    noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)

                    # Get the text embedding for conditioning
                    encoder_hidden_states = text_encoder(batch["input_ids"])[0] #Loke: text embedding
                    ...
                    if args.with_prior_preservation:
                        ...
                        target, target_prior = torch.chunk(target, 2, dim=0) #Loke: ?, the 
                        ...
                        loss = loss + args.prior_loss_weight * prior_loss #Loke: loss
                    else:
                        loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean") #Loke: loss

                    #############
                    #update loss
                    #if in last few steps

                    if (args.max_train_steps - 1 - step) <= 10:
                        image_denoised = latents_to_pil(noisy_latents - model_pred)[0]
                        image_path = f"denoised_image_{step}.png"
                        image_denoised.save(image_path)
                        l1 = loss_module(args.instance_prompt, [image_path]) #instance prompt - image
                        
                        #seg_image = seg(image_denoised) #segmentation
                        #seg_path = f"segmented_image_{step}.png"
                        #seg_image.save(seg_path)
                        #l2 = loss_module(args.class_prompt, [seg_path]) #class prompt - segmented image
                        loss += w1 * l1 #+ w2 * l2 #w1/w2: weight for l1 & l2
